...
const DEFAULTS = {
  hero:    { title: "Immigration Consultants Serving Toronto", breadcrumb: "Toronto" },
  content: {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Bar chart">
      {yTicks.map((tick) => {
        ...
        return (
          <g key={tick}>
            <line x1={pad.left} x2={pad.left + chartW} y1={y} y2={y} stroke="#e5e7eb" strokeWidth="1" strokeDasharray="4 3" />
            <text x={pad.left - 6} y={y + 4} textAnchor="end" fontSize="9" fill="#888">
              {tick === 0 ? "0" : `${tick / 1000}k`}
            </text>
          </g>
        );
      })}
      <line x1={pad.left} x2={pad.left + chartW} y1={pad.top + chartH} y2={pad.top + chartH} stroke="#d1d5db" strokeWidth="1.5" />
      {BAR_DATA.map((d, i) => {
        ...
        return (
          <g key={d.label}>
            <rect x={x} y={y} width={bw} height={barH} fill="#cc1f1f" rx="3" opacity="0.92" />
            <text x={midX} y={y - 4} textAnchor="middle" fontSize="8" fill="#cc1f1f" fontWeight="700">
              {(d.value / 1000).toFixed(1)}k
            </text>
            {d.label.split(" ").map((word, wi) => (
              <text key={wi} x={midX} y={pad.top + chartH + 14 + wi * 11} textAnchor="middle" fontSize="8.5" fill="#555">
                {word}
              </text>
            ))}
          </g>
    ...
    <svg viewBox={`0 0 ${W} ${H}`} xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Line chart">
      ...
      {yTicks.map((tick) => {
        ...
        return (
          <g key={tick}>
            <line x1={pad.left} x2={pad.left + chartW} y1={y} y2={y} stroke="#e5e7eb" strokeWidth="1" strokeDasharray="4 3" />
            <text x={pad.left - 8} y={y + 4} textAnchor="end" fontSize="9" fill="#888">{`${tick / 1000}k`}</text>
          </g>
        );
      })}
      <line x1={pad.left} x2={pad.left + chartW} y1={pad.top + chartH} y2={pad.top + chartH} stroke="#d1d5db" strokeWidth="1.5" />
      <path d={areaD} fill="url(#lineGrad)" />
      <path d={pathD} fill="none" stroke="#cc1f1f" strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" />
      {pts.map((p, i) => (
        <g key={i}>
          <circle cx={p.x} cy={p.y} r="4.5" fill="#fff" stroke="#cc1f1f" strokeWidth="2.5" />
          <text x={p.x} y={p.y - 9} textAnchor="middle" fontSize="8" fill="#cc1f1f" fontWeight="700">
            {`${Math.round(p.value / 1000)}k`}
          </text>
          {p.label.split("\n").map((word, wi) => (
            <text key={wi} x={p.x} y={pad.top + chartH + 15 + wi * 11} textAnchor="middle" fontSize="8.5" fill="#555">
              {word}
            </text>
          ))}
        </g>
...
function MapleLeaf() {
  return (
    <svg className="zfc-tor-maple" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <path
        d="M50 5 C50 5 44 18 38 20 C32 22 20 16 20 16 C20 16 26 26 24 32 C22 38 10 40 10 40 C10 40 20 46 20 52 C20 58 14 68 14 68 C14 68 26 64 32 66 C38 68 42 80 42 80 L46 65 L50 68 L54 65 L58 80 C58 80 62 68 68 66 C74 64 86 68 86 68 C86 68 80 58 80 52 C80 46 90 40 90 40 C90 40 78 38 76 32 C74 26 80 16 80 16 C80 16 68 22 62 20 C56 18 50 5 50 5Z"
        stroke="#cc1f1f" strokeWidth="3" strokeLinejoin="round" fill="rgba(204,31,31,0.08)"
      />
...
export default function TorontoPage() {
  const raw = getPageContent("toronto") as typeof DEFAULTS;
  const c = {
    hero:    { ...DEFAULTS.hero,    ...(raw?.hero    ?? {}) },
    content: {
      title:      raw?.content?.title      ?? DEFAULTS.content.title,
      paragraphs: raw?.content?.paragraphs ?? DEFAULTS.content.paragraphs,
    },
    ...
    <div className="zfc-toronto-page">
      ...
      <section className="zfc-tor-content" aria-label="About Toronto immigration services">
        <div className="zfc-tor-content__inner">
          <MapleLeaf />
          <h2 className="zfc-tor-title">{c.content.title}</h2>
          <div className="zfc-tor-body">
            {c.content.paragraphs.map((para, i) => (
              <p key={i}>{para}</p>
            ))}
          </div>
        </div>
      ...
      <section className="zfc-tor-charts" aria-label="Toronto immigration statistics">
        <div className="zfc-tor-charts__inner">
          <div className="zfc-tor-chart-card">
            <h3 className="zfc-tor-chart-card__title">Top 10 Countries of Birth Among Immigrants in Toronto (2016–2021)</h3>
            <BarChart />
          </div>
          <div className="zfc-tor-chart-card">
            <h3 className="zfc-tor-chart-card__title">Toronto Immigration Trends (1980–2021)</h3>
            <LineChart />
          </div>
        </div>
      </section>
        ...
        <div className="zfc-tor-contact__inner">
          <div className="zfc-tor-contact__item">
            ...
            <span className="zfc-tor-contact__value">
              <a href={`tel:${c.contact.phone.replace(/\D/g, "").replace(/^/, "+")}`}>{c.contact.phone}</a>
            </span>
          </div>
          <div className="zfc-tor-contact__item">
            <div className="zfc-tor-contact__icon-wrap">
